...
export const clientService = {
  ...
  subscribeToClients(
    ...
    const q = query(
      ...
      where('deleted', '==', false),
      orderBy('created_at_local', 'desc') // Use local fallback for consistent sorting across devices
    );
    return onSnapshot(
      q,
  ...
  async createClient(
    name: string,
    profile?: Partial<ClientWithProfile>
  ): Promise<string> {
    const search_tokens = generateSearchTokens(name);

    // Ensure client_uuid is present (generated by store, but enforced here)
    if (!profile?.client_uuid) {
      throw new Error('client_uuid is required for creation');
    }
    ...
    return dbService.appendOnly(colRef, payload as any);
  },
